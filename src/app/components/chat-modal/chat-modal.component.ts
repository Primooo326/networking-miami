...
export class ChatModalComponent implements OnInit, AfterViewInit, OnDestroy {
        ...
        for (const element of data.results) {
          if (element.estado != 'visto') {
            ...
            res2.subscribe(
              ...
              (err) => {
                console.log(
                  '🚀 ~ file: chat-modal.component.ts:102 ~ ChatModalComponent ~ res.subscribe ~ err:',
                  err
                );
              }
            );
    ...
    if (data.conversacion_id === this.conversacion_id) {
      if (data.estado != 'visto') {
        ...
        res.subscribe(
          ...
          (err) => {
            console.log(
              '🚀 ~ file: chat-modal.component.ts:190 ~ ChatModalComponent ~ handleNewMessage ~ err:',
              err
            );
          }
        );
  ...
  async toggleFijado() {
    ...
    await this.matchSrvc
      .updateMatch(this.user.contactoDb_id, fijado)
      .then((data: any) => {
      ...
      .catch((err: any) => {
        console.log(
          '🚀 ~ file: chat-modal.component.ts:208 ~ ChatModalComponent ~ awaitthis.matchSrvc.updateMatch ~ err:',
          err
        );
      });
  }
